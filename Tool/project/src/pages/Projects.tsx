...
export function Projects() {
  ...
  const handleCreateProject = async (e: React.FormEvent) => {
    ...
    setNewProject({ name: "", description: "", client_id: "", priority: "medium", status: "not_started", deadline: "", monthly_hour_allocation: 40, tags: [], owner_id: "" });
    setNewTag("");
    // Call API
    try {
      const payload: Partial<ApiProject> & { client?: string; monthlyHours?: number; tags?: string[]; owner?: string } = {
        name: newProject.name,
        description: newProject.description,
        createdBy: user._id!,
        deadline: newProject.deadline,
      ...
      if (created) {
        ...
        dispatch({ type: "SET_PROJECTS", payload: original.map(p => p.id === tempId ? real : p) });
      }
    } catch (error) {
